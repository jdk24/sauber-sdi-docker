...
if [ ${NEW_STRUCT_TS} -gt ${LATEST_BK_TS} ] ; then
    ...
    if PGPASSWORD=$POSTGRES_INIT_PASSWORD pg_dump -s -h db -U postgres -d sauber_data > db_backup/$OUTFILE.sql; then
        if PGPASSWORD=$POSTGRES_INIT_PASSWORD psql -h db -U postgres -d sauber_data -c 'SELECT timescaledb_pre_restore();' ; then 
                if PGPASSWORD=$POSTGRES_INIT_PASSWORD psql -h db -U postgres < db_structure/$NEW_STRUCT ; then
                    if PGPASSWORD=$POSTGRES_INIT_PASSWORD psql -h db -U postgres -d sauber_data -c 'SELECT timescaledb_pre_restore();' ; then
